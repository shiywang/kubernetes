...
var _ = Describe("Custom Resource Definition resources [Flaky] [Disruptive]", func() {
	...
	f := framework.NewDefaultFramework("customresourcedefinitions")
	...
	Context("Simple Custom Resource Definition", func() {
		It("creating/deleting custom-resource-definition objects works [Conformance]", func() {
			defer func() {
				if err := apiExtensionClient.Apiextensions().CustomResourceDefinitions().Delete(crd.Name, nil); err != nil {
					framework.Failf("failed to delete custom resource definitions resource: %v", err)
				}
			}()


			if _, err := apiExtensionClient.Apiextensions().CustomResourceDefinitions().Create(crd); err != nil {
				framework.Failf("failed to create custom resource definitions resource: %v", err)
			}

			wait.Poll(time.Second*30, time.Minute*5, func() (bool, error) {
				data, err := f.ClientSet.Extensions().RESTClient().Get().AbsPath("/apis/company.com/v1beta1/foos").DoRaw()
				if err != nil {
					return false, err
				}
			...
			if _, err := f.ClientSet.Extensions().RESTClient().Post().AbsPath("/apis/company.com/v1beta1/namespaces/default/foos").Body(bodyData).DoRaw(); err != nil {
				framework.Failf("failed to create: %v", err)
			}

			data, err = f.ClientSet.Extensions().RESTClient().Get().AbsPath("/apis/company.com/v1beta1/namespaces/default/foos/foo").DoRaw()
			if err != nil {
			...
			if _, err := f.ClientSet.Core().RESTClient().Delete().
				AbsPath("/apis/company.com/v1beta1/namespaces/default/foos/foo").
