...
func TestIsList(t *testing.T) {
	...
	for _, item := range tests {
		if e, a := item.isList, IsListType(item.obj); e != a {
...
func TestEachListItem(t *testing.T) {
	...
	for i, test := range testCases {
		list := []runtime.Object{}
		err := EachListItem(test.in, func(obj runtime.Object) error {
...
func TestSetExtractListRoundTrip(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		list, err := ExtractList(start)
		if err != nil {
			t.Errorf("Unexpected error %v", err)
			continue
		}
		got := &api.PodList{}
		err = SetList(got, list)
		if err != nil {
			t.Errorf("Unexpected error %v", err)
			continue
		}
