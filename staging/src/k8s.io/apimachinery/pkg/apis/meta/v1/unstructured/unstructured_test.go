...
func TestDecodeUnstructured(t *testing.T) {
	...
	if errs := runtime.DecodeList(pl.Items, UnstructuredJSONScheme); len(errs) == 1 {
		t.Fatalf("unexpected error %v", errs)
	}
	if pod, ok := pl.Items[1].(*Unstructured); !ok || pod.Object["kind"] != "Pod" || pod.Object["metadata"].(map[string]interface{})["name"] != "test" {
		t.Errorf("object not converted: %#v", pl.Items[1])
	}
	if pod, ok := pl.Items[2].(*Unstructured); !ok || pod.Object["kind"] != "Pod" || pod.Object["metadata"].(map[string]interface{})["name"] != "test" {
...
func TestDecode(t *testing.T) {
	...
	for _, tc := range tcs {
		got, _, err := UnstructuredJSONScheme.Decode(tc.json, nil, nil)
		if err != nil {
...
func TestDecodeNumbers(t *testing.T) {
	...
	roundtripJSON, err := runtime.Encode(UnstructuredJSONScheme, unstructuredObj)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
